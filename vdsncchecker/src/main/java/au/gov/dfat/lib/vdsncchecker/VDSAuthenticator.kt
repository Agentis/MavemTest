...
open class VDSAuthenticator : IVDSAuthenticator {
    ...
    override fun verifyVDS(vds: VDS, certificateRepository: ICertificateRepository): Boolean{
        ...
        if(cscaData != null){
            ...
            val tempList = ArrayList<ByteArray>()
            tempList.add(cscaData.clr)

            return verifyVDS(vds, cscaData.certificate,  cscaData.hash, tempList)
        }

        return false
    }

    private fun verifyCSCAExists(certRepo: ICertificateRepository, vds: VDS) : CSCAData?{
        val bscCertData = getBSCCertDataFromVDS(vds)
        var data = certRepo.findIssuer(bscCertData, certFactory)
        if(data != null){
            return CSCAData(data.hash, data.certificate, data.clr)
        }
    ...
    protected open fun verifyBSCCertAKIMatchesCSCACertAKI(vds: VDS, cscaCertData: ByteArray) {
        ...
        try {
            // Get BSC AKI
            val bscAKIFullExtVal = bscCert.getExtensionValue(akiExtOid)
            val bscAKIExtVal = ASN1OctetString.getInstance(bscAKIFullExtVal).octets
            val bscAKI = AuthorityKeyIdentifier.getInstance(bscAKIExtVal)
            val bscAKIKeyIdentifier = bscAKI.keyIdentifier.asList()

            // Get CSCA AKI
            val cscaAKIFullExtVal = cscaCert.getExtensionValue(akiExtOid)
            val cscaAKIExtVal = ASN1OctetString.getInstance(cscaAKIFullExtVal).octets
            val cscaAKI = AuthorityKeyIdentifier.getInstance(cscaAKIExtVal)
            val cscaAKIKeyIdentifier = cscaAKI.keyIdentifier.asList()

            // Check if BSC certificate's AKI matches CSCA certificate's AKI
            if (bscAKIKeyIdentifier != cscaAKIKeyIdentifier) {
                throw Exception()
            }
        } catch (_: Exception) {
    ...
    private val certFactory: CertificateFactory
        get() {
            return try { CertificateFactory.getInstance("X.509") } catch (_: Exception) { null }
